...
fn main() -> std::io::Result<()> {
    ...
    let tokenizer = Parser::new_tokenizer(CharIterator::new(2044,ioutils::mk_buf_reader(filepath)?));
    for t in tokenizer {
        println!("{:?}", t);
    }
    println!("Tokenizer took: {:?}", start.elapsed());

    let mut parser = Parser::new(CharIterator::new(2044,ioutils::mk_buf_reader(filepath)?));
